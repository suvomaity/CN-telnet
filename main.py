    ...
    while True:
        ...
        if operation[0] == "open":
            ...
            if not connected_socket:
                ...
                if (
                    len(operation) > 3 and operation[3] == "-e"
                ...
                while True:
                    ...
                    if command[0] == "send" and command[1] == "message":
                        ...
                        send_message(
                            connected_socket,
                            command_to_send + (CHUNK_SIZE - len(command_to_send)) * " ",
                        )
                        ...
                        send_message(
                            ...
                            "upload"
                            + fileName
                            + (CHUNK_SIZE - len(fileName) - 6) * " ",
                        )
                        success = send_file(connected_socket, command[1])
                        (
                            print("Transmision was successful.")
                            if success
                            else print("Transmision failed.")
                        )
                        print(recv_message(connected_socket))
    ...
    try:
        ...
        while True:
            ...
            client_thread = threading.Thread(
                target=client_handler, args=(connection, info, keys)
            )
            client_thread.start()
...
def client_handler(given_socket: socket.socket, info: tuple, keys):
    ...
    try:
        ...
        while result:
            if len(keys) > 0:
                result = recv_and_proccess(
                    given_socket, keys["private"], client_public_key
                )
            else:
...
def send_message(given_socket: socket.socket, message: str, public_key=None):
    ...
    encoded_message = (
        ...
        if not public_key
        else rsa.encrypt(message.encode(ENCODING), public_key)
    )
    if LOG_MODE:
...
def send_message_to_host(
    given_socket: socket.socket, public_key=None, client_private_key=None
):
...
def recv_and_proccess(
    given_socket: socket.socket, private_key=None, client_public_key=None
):
    ...
    try:
        ...
        # FIXED: Strip trailing spaces from padding
        first_run = first_run.strip()

        if first_run.startswith("upload"):
            name = first_run.replace("upload", "").strip()  # FIXED: Strip
            ...
            cmd = first_run[len("exec ") :].strip()  # FIXED: Strip before split
            ...
            path = first_run.replace("download", "").strip()  # FIXED: Strip
            ...
            if private_key:
                while not got_message:
                    got_message = rsa.decrypt(
                        given_socket.recv(CHUNK_SIZE), private_key
                    ).decode(ENCODING)
            else:
                while not got_message:
            ...
            print(got_message)
            message = (
                f"[Server response] Got {len(got_message)} bytes of your message.\n"
            )
    ...
    while ip_start <= ip_end:
        for port in range(1, 1024):
            ...
            if check_port(str(ip_start), port):
                ...
                comment = (
                    PORTS_DIC[str(port)]["comment"] if str(port) in PORTS_DIC else ""
                )
...
def add_history(connection, command: str):
    ...
    if connection is None:  # <--- ADD THIS CHECK
        return  # <--- EXIT IF CONNECTION IS INVALID

    try:
        cursor = connection.cursor()
        cursor.execute(
            """INSERT INTO history (used_date, command) VALUES (%(date_time)s,
                       %(command)s);""",
            {"date_time": str(datetime.now()), "command": command},
        )
        connection.commit()
    except Exception as exc:
        print(f"Database {exc}")
